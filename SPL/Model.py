...
#
#     CBAM 结合了通道注意力和空间注意力机制，用于增强特征图的表示能力。
#
#     Args:
#         num_feat (int): 输入特征图的通道数。
#         reduction (int): 通道注意力中 MLP 的通道缩减比率，默认值为 16。
#         spatial_kernel (int): 空间注意力中卷积核的大小，默认值为 7。
#     """
#
#     def __init__(self, num_feat, reduction=16, spatial_kernel=5):
#         super(CBAM, self).__init__()
#
#         # 通道注意力模块
#         self.mlp = nn.Sequential(
#             nn.Conv2d(num_feat, num_feat // reduction, 1, bias=False),  # 降维
#             nn.ReLU(inplace=True),  # 激活函数
#             nn.Conv2d(num_feat // reduction, num_feat, 1, bias=False)  # 升维
#         )
#
#         # 空间注意力模块
#         self.conv = nn.Conv2d(2, 1, kernel_size=spatial_kernel,
#                               padding=spatial_kernel // 2, bias=False)  # 输入通道为 2（平均池化和最大池化拼接）
#
#     def forward(self, x):
#         # 通道注意力
#         avg_pool = F.adaptive_avg_pool2d(x, 1)  # 自适应平均池化到 1x1
#         max_pool = F.adaptive_max_pool2d(x, 1)  # 自适应最大池化到 1x1
#         channel_attention = torch.sigmoid(self.mlp(avg_pool) + self.mlp(max_pool))  # 共享 MLP，求和后激活
#         channel_attended = x * channel_attention  # 应用通道注意力权重
#
#         # 空间注意力
#         avg_pool_spatial = torch.mean(channel_attended, dim=1, keepdim=True)  # 沿通道轴平均池化
#         max_pool_spatial, _ = torch.max(channel_attended, dim=1, keepdim=True)  # 沿通道轴最大池化
#         concat = torch.cat([avg_pool_spatial, max_pool_spatial], dim=1)  # 拼接
#         spatial_attention = torch.sigmoid(self.conv(concat))  # 卷积后激活
#         output = channel_attended * spatial_attention  # 应用空间注意力权重
#
#         return output
#
#
# class CAB(nn.Module):
#     """Channel Attention Block with CBAM.
#
#     该模块包含两个卷积层和一个 CBAM 注意力模块，用于特征提取和增强。
#
#     Args:
#         num_feat (int): 输入特征图的通道数。
#         compress_ratio (int): 第一个卷积层中的通道压缩比率，默认值为 3。
#         squeeze_factor (int): CBAM 中通道注意力的缩减比率，默认值为 30。
#     """
#
#     def __init__(self, num_feat, compress_ratio=3, squeeze_factor=30):
#         super(CAB, self).__init__()
#         self.cab = nn.Sequential(
#             nn.Conv2d(num_feat, num_feat // compress_ratio, 3, 1, 1),  # 第一个卷积层，压缩通道
#             nn.GELU(),  # GELU 激活函数
#             nn.Conv2d(num_feat // compress_ratio, num_feat, 3, 1, 1),  # 第二个卷积层，恢复通道
#             CBAM(num_feat, reduction=squeeze_factor)  # CBAM 注意力模块
#         )
#
#     def forward(self, x):
#         return self.cab(x)
